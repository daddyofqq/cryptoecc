...
struct NAFw_accessor {
    struct naf_iterator {
        naf_iterator(char* ptr, size_t size, bool dir) : ptr(ptr), size(size), dir(dir)
        {
            if (dir) {
                current = ptr;
            } else {
        ...
        {
            return current < ptr || (current >= ptr + size);
        }
    ...
    auto get_read_iterator()
    {
        ...
        return naf_iterator(&m[0], i + 1, false);
    }

    auto get_write_iterator()
    {
        return naf_iterator(&m[0], N, true);
    }
...
template <unsigned N>
struct NAFw_accessor<2, N> {
    template <typename IT>
    struct naf_iterator {
        naf_iterator(IT i1, IT i2) : i1(i1), i2(i2){};
    ...
    auto get_read_iterator()
    {
        bit_iterator i1(m1);
        bit_iterator i2(m2);
        while (!i1.end() && !i2.end() && !i1 && !i2) {
            i1.next();
            i2.next();
        }
        return naf_iterator(i1, i2);
    }

    auto get_write_iterator()
    {
        bit_iterator i1(m1, false);
        bit_iterator i2(m2, false);
        return naf_iterator(i1, i2);
    }
...
template <unsigned N, unsigned W = 2>
struct NAF {
    static constexpr unsigned WINDOW_SIZE = W;

    NAF(mpi<N> const& m)
    {
        mpi<N> k(m);
        mpi<1> one(1);
        auto it = accessor.get_write_iterator();

        while (k.compare(one) >= 0) {
            if (k.is_odd()) {
                int ki = static_cast<int>(k[0] & ((1 << WINDOW_SIZE) - 1));
                if (ki >= (1 << (WINDOW_SIZE - 1))) {
                    ki -= (1 << WINDOW_SIZE);
                }
                k.sub(mpi<bits_per_limb>(ki));
                it.set(ki);
            } else {
                it.set(0);
            }
    ...
    auto get_iterator()
    {
        return accessor.get_read_iterator();
    }

    NAFw_accessor<W, N + 1> accessor;
};
